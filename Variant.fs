...
type Variant(value: VariantValue) =
    ...
    static member op_BitwiseNot (a: Variant) =
        ...
        | VarChar x -> Variant(VarUInt32(~~~uint32(x)))
        | VarInt8 x -> Variant(VarInt8(~~~x))
        | VarUInt8 x -> Variant(VarUInt8(~~~x))
        | VarInt16 x -> Variant(VarInt16(~~~x))
        | VarUInt16 x -> Variant(VarUInt16(~~~x))
        | VarInt32 x -> Variant(VarInt32(~~~x))
        | VarUInt32 x -> Variant(VarUInt32(~~~x))
        | VarInt64 x -> Variant(VarInt64(~~~x))
        | VarUInt64 x -> Variant(VarUInt64(~~~x))
        | VarSingle x -> Variant(VarSingle(float32(~~~int32(roundDouble(double x)))))
        | VarDouble x -> Variant(VarDouble(double(~~~int64(roundDouble(x)))))
        | VarDecimal x -> Variant(VarDecimal(decimal(~~~int64(roundDecimal(x)))))
        | _ -> notSupported()
